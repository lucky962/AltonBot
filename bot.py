            ...
            if ('Executive Team' in roles) or ('Management Team' in roles) or ('High Rank Team' in roles):
                ...
                for i in warnings:
                    ...
                    try:
                        ...
                        try:
                            msg.append('*' + parts[0] + '* was warned by *' + str(message.guild.get_member(int(parts[1])).nick) + '* for reason: ' + parts[2])
                        except AttributeError:
                            try:
                                msg.append('*' + str(message.guild.get_member(int(parts[0])).nick) + '* was warned by *' + parts[1] + '* for reason: ' + parts[2])
                            except AttributeError:
